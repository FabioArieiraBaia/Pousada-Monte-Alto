...
export default function BookingModal({ room, initialDates = {}, isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-stone-900/80 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-fade-in">
          ...
          {submittedResult ? (
            ...
            <div className="text-center py-6 space-y-4">
              ...
              <div className="bg-sand-50 p-4 rounded-2xl border border-sand-200 text-left text-xs space-y-1.5 text-stone-700">
                ...
                <p><strong>Valor Estimado:</strong> {formatCurrency(estimatedTotal)}</p>
              </div>
              ...
              <div className="pt-4 flex flex-col gap-2.5">
                {submittedResult.whatsapp_url && (
                  <a
                    ...
                    className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3.5 px-6 rounded-2xl shadow-lg flex items-center justify-center gap-2 transition-transform hover:scale-[1.02]"
                  >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-amber-50/80 p-3.5 rounded-2xl border border-amber-200/80 flex items-center justify-between">
                <div>
                  <span className="text-[11px] text-stone-600 block">
                    {t('booking.totalEstimate')} ({t('booking.nightsCount', { count: nights })})
                  </span>
                  <span className="text-xl font-bold text-amber-700 font-serif">
                    {formatCurrency(estimatedTotal)}
                  </span>
                </div>
                <span className="text-[10px] text-amber-800 bg-amber-200/60 px-2 py-1 rounded-md font-semibold">
                  Sem taxas extras
                </span>
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-amber-500 hover:bg-amber-600 active:scale-[0.99] text-stone-950 font-bold py-3.5 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 uppercase tracking-wider text-xs"
              >
                {loading ? (
